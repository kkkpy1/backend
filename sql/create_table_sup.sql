...
CREATE TABLE sup.member (
	...
	member_phone VARCHAR2(11) NOT NULL, /* 전화번호 */
	member_activated CHAR(1) DEFAULT 1 NOT NULL, /* 활성화 구분 */
	member_code CHAR(1) DEFAULT 1 NOT NULL, /* 회원 구분 */
	member_business_id VARCHAR2(10), /* 사업자등록번호 */
	member_address VARCHAR2(200), /* 주소 */
	member_account VARCHAR2(30), /* 계좌번호 */
	member_terms CHAR(1) DEFAULT 1 NOT NULL, /* 약관동의여부 */
	member_date DATE DEFAULT SYSDATE NOT NULL /* 가입 일자 */
);
...
COMMENT ON COLUMN sup.member.member_code IS 'A = admin
S = seller
C = customer';
...
CREATE TABLE sup.product (
	...
	product_img VARCHAR2(300) NOT NULL, /* 상품 이미지 */
	...
	product_activated CHAR(1) DEFAULT 1 NOT NULL, /* 활성화 구분 */
	product_date DATE DEFAULT SYSDATE NOT NULL /* 업데이트 일자 */
);
...
CREATE TABLE sup.purchase (
	...
	purchase_state CHAR(1) DEFAULT 0 NOT NULL, /* 주문 상태(사용/미사용/거절) */
...
/* 친구관계목록 */
CREATE TABLE sup.friends (
	friends_id NUMBER(38) NOT NULL, /* 친구관계 기본키 */
	member_id VARCHAR2(20) NOT NULL, /* 회원 id */
	friend_id VARCHAR2(20) NOT NULL, /* 친구의 id */
	friends_date DATE DEFAULT SYSDATE NOT NULL /* 갱신 날짜 */
);

COMMENT ON TABLE sup.friends IS '친구관계목록';

COMMENT ON COLUMN sup.friends.friends_id IS '친구관계 기본키';

COMMENT ON COLUMN sup.friends.member_id IS '회원 id';

COMMENT ON COLUMN sup.friends.friend_id IS '친구의 id';

COMMENT ON COLUMN sup.friends.friends_date IS '갱신 날짜';

CREATE UNIQUE INDEX sup.PK_friends
	ON sup.friends (
		friends_id ASC
	);

ALTER TABLE sup.friends
	ADD
...
CREATE TABLE sup.report (
	...
	report_checked CHAR(1) DEFAULT 0 NOT NULL, /* 글확인여부 */
...
CREATE TABLE sup.post (
	...
	post_shared_range CHAR(1) DEFAULT 1 NOT NULL, /* 공유 범위 */
	post_like_count NUMBER(38) DEFAULT 0 NOT NULL, /* 글 좋아요 수 */
	post_date DATE DEFAULT SYSDATE NOT NULL /* 작성 날짜 */
);
...
CREATE TABLE sup.like_post (
	...
	like_post_date DATE DEFAULT SYSDATE NOT NULL /* 좋아요 날짜 */
);
...
CREATE TABLE sup.like_product (
	...
	like_product_date DATE DEFAULT SYSDATE NOT NULL /* 좋아요 날짜 */
);
...
CREATE TABLE sup.wishlist (
	...
	wishlist_date DATE DEFAULT SYSDATE NOT NULL /* 업데이트 날짜 */
);
...
CREATE TABLE sup.ranking (
	...
	ranking_date DATE DEFAULT SYSDATE NOT NULL /* 업데이트 날짜 */
);
...
CREATE TABLE sup.qna (
	...
	qna_date DATE DEFAULT SYSDATE NOT NULL, /* 갱신 날짜 */
	qna_group NUMBER(3), /* 그룹 */
	qna_level NUMBER(3), /* 깊이 */
	qna_step NUMBER(3) /* 나열순서 */
);
...
COMMENT ON COLUMN sup.qna.qna_group IS '그룹';

COMMENT ON COLUMN sup.qna.qna_level IS '깊이';

COMMENT ON COLUMN sup.qna.qna_step IS '나열순서';
...
ALTER TABLE sup.friends
	ADD
		CONSTRAINT FK_member_TO_friends
		FOREIGN KEY (
			member_id
		)
		REFERENCES sup.member (
...
ALTER TABLE sup.friends
	ADD
		CONSTRAINT FK_member_TO_friends2
		FOREIGN KEY (
			friend_id
		)
		REFERENCES sup.member (
